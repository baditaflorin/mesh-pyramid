...
The image above (`docs/preview.png`) is a stitched screenshot of two peers in the same room, generated by `npm run demo` which:

1. Builds the app and starts a local preview server.
2. Opens two Playwright pages (peer A + peer B) in the same browser context — they mesh-sync via y-webrtc's BroadcastChannel fallback.
3. Runs `tests/demo/scenario.mjs` to drive both peers.
4. Records each peer's screen to `docs/demo-a.webm` and `docs/demo-b.webm`.
5. Composites a side-by-side static `docs/preview.png` via a stitcher page (no ffmpeg required).
